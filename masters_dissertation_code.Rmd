...
```{r, logistic regression models - modelling}
...
epochs_vec <- round(seq(from = 5, to = 75, length.out = 10), 0)
...
epochs_vec <- round(seq(16, 30, length.out = 3),0)
batch_size_vec <- c(80, 90, 100)
dense_units_vec <- c(8,16,32,64,128)
...
# Note: dropout did not perform as well as L2 regularisation. Lets look at some of the distributions

runs_2 <- ls_runs(runs_dir = "experiment_2_runs")
runs_3 <-  ls_runs(runs_dir = "experiment_3_runs") 
...
epochs_vec <- c(23)
batch_size_vec <- c(100)
dense_units_vec <- c(32,64)
lambda_vec <- c(0.01, 0.1)
...
runs_4_plot  %>% 
  ggplot(aes(x = flag_optimizer , y = metric_val_acc , fill = as.factor(flag_optimizer))) +
  geom_boxplot() +
...
# Note: From the hyper-parameter tuning two models are goin to be trained and tested 
