...
write.csv()
example_data_set <- read.csv("diabetes.csv")
...
# 5. Hyper- parameter tuning of threshold value
...
optimal_threshold_PR <- function(model_performance){
  ...
  PR_threshold_res <- list(optimal_threshold = optimal_threshold,
                           recall = model_performance@y.values[[1]][max_gmean_ind],
                           precision = model_performance@x.values[[1]][max_gmean_ind],
...
# 5.3 Calculating performance of each model based on precision-recall curves and detetrmnig optimal threshold value
...
# Regular LR model
pi_hat_log_std_network_test <- predict(log_std_model_network, newdata = test_network_features[,-1], type = "response")

# LASSO LR model
pi_hat_log_lasso_network_test <- predict(log_cv_model_network, 
                                    newx = as.matrix(test_network_features[,-c(1,ncol(test_network_features))]),
                                    s = log_cv_model_network$lambda.1se,
                                    type = "response")
...
# Regular LR model
pi_hat_log_std_trans_test <- predict(log_std_model_trans, newdata = test_transactional_features[,-1], type = "response")

# LASSO LR model
pi_hat_log_lasso_trans_test <- predict(log_cv_model_trans, 
                                    newx = as.matrix(test_transactional_features[,-c(1,ncol(test_transactional_features))]),
                                    s = log_cv_model_trans$lambda.1se,
                                    type = "response")


# 7.1.3 all features
pi_hat_log_std_all_test <- predict(log_std_model_all, newdata = test_all_features[,-1] , type = "response")

# LASSO LR model
pi_hat_log_lasso_all_test <- predict(log_cv_model_all, 
                                    newx = as.matrix(test_all_features[,-c(1,ncol(test_all_features))]),
                                    s = log_cv_model_all$lambda.1se,
                                    type = "response")
...
```{r, data pre-processing -  neural network models}
...
test_model %>% 
  layer_dense(units = 36, activation = "relu", input_shape = c(27)) %>% 
  layer_dense(units = 2, activation = "softmax")
# 2. compiling model - specifying loss and optimizer
...
# 4. evaluation of model (predicting test set values)

test_model %>% evaluate(test_network_features_nn , test_target_nn)

# Display conduction matrix

Y_test_hat <- predict_classes(test_model, test_network_features_nn)
table(test_network_features$is_fraud, Y_test_hat)
...
```{r, hyper-parameter tuning -  neural network models}
...
runs_1_plot  %>% 
  ...
  geom_boxplot() +
  labs(x = "Batch size", y = "Validation accuracy") +
...
# Finding: i) Overall the val accuracy seems to stay stable for most unit sizes. ii) Generally, for all units the validation accuracy slightly increases with higher batch sizes (especially seen with units = 32).
...
runs_1_plot  %>% 
  ...
  geom_boxplot() +
  labs(x = "Epochs", y = "Validation accuracy") +
  facet_wrap(flag_dense_units~.) +
  scale_fill_brewer(palette = "Set3") +
  stat_summary(fun = median, geom = "line", aes(group = 1), col = "red", size = 1) +
  stat_summary(fun = q1_func, geom = "line", aes(group = 1), col = "red", size = 0.85, linetype = "dashed") +
...
# Finding: i) Overall the val accuracy seems to decrease as the number of epochs increase (especially for units 32,64,128). ii) Generally, there  is a slight peak at 16 and 26 epochs (see 8,16,32).
...
# NB! Include these plots in Appendix and mention that with the first set of hyper-parameters that were tuned plots such as these were also made, but no real deduction could be made from them (deductions could be made in how the model genaralises based on the tuned hyper-parameters). The dashed lines were arbitrarily chosen to show that good performance would be in the upper-right quadrant (high-training accuracy and high validation accuracy) 
