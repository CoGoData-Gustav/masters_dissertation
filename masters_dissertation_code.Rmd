...
nn_1 <- ls_runs(runs_dir = 'nn_1_high_level_T' ) 
nn_2 <- ls_runs(runs_dir = 'nn_2_high_level_T')
...
bar_test_balanced_acc <- results_df %>% 
  ...
  scale_fill_brewer(palette = "Dark2", name = " ", labels = c("Logistic regression (PR)", "Logistic regression (ROC)", "(8)-Network") ) +
  geom_text(aes(label = round(test_balanced_acc,3)), position = position_dodge(width = 0.9), vjust = -0.1, size = 3) + 
  theme_bw() + 
  theme(legend.position = "none",
        text = element_text(size = 10)) +
...
ggarrange(
  bar_test_acc,     
  ggarrange(bar_test_f1, bar_test_balanced_acc, ncol = 2 ), 
  nrow = 2) 
...
conf_mat <- log_test_results$lasso_network_ROC

conf_mat

# test accuracy

(1182+69)/(1182+69+3+56) # correct


func_res <- LR_perf_metrics_func (conf_mat)
...
2*(((69)/(69+3))*((69)/(69+56)))/((69)/(69+3) + (69)/(69+56))


# balanced accuracy 
func_res$balanced_acc

(1182/(1182+56) + 69/(69+3))/2
...
transaction_df <- testing_transactions_final
...
### validation data_combined %>%

min_lambda_1 <- nn_1$flag_lambda[which(nn_1$metric_val_loss == min(nn_1$metric_val_loss))]
min_lambda_2 <- nn_2$flag_lambda[which(nn_2$metric_val_loss == min(nn_2$metric_val_loss))]

data_combined <- data.frame(lambda = c(nn_1$flag_lambda, nn_2$flag_lambda),
                            model = c(rep("Model 1",nrow(nn_1)), rep("Model 2",nrow(nn_2))),
                            val_loss = c(nn_1$metric_val_loss, nn_2$metric_val_loss) 
)
