...
coef_df %>% 
  ggplot(aes(y = coef_value, x = log_lambda_value, col = predictors)) +
  geom_line() +
  ...
  xlim(-15, max(log(log_LASSO_model_network$lambda))) +
...
```{r, model construction and testing - response curve generation}
...
#(build other models here! Ttansactional and Combined)
...
# 1. Network data (NB! Going to have to do for trans and combined features)
...
for (i in 1:(M*M-1)) {
  ...
  numeric_final_rep_df <- rbind(numeric_final_rep_df,numeric_values )
...
two_value_variables <- c("total_degree", "non_fraud_degree", "out_degree", "out_degree_non_fraud")
...
fixed_values_df <- final_rep_df_list[[1]]
...
for(i in 1:dim(train_x)[2]){
  ...
  for(j in (k+1):dim(train_x)[2]){
    ...
    if(sum(pred$fraud > 0.65) > 0 & sum(pred$non_fraud > 0.65)){ # condition when there is evidence of variables being related to fraud
      
      # saving variables in diagnostic df
      diagnostic_df$x_axis_predictor[p] <- col_names[i]
      diagnostic_df$y_axis_predictor[p] <- col_names[j]
      ...
      # generating plots (commented out to run faster)
    ...
    current_plot <-  plot_df %>%
      ...
      labs(x = col_names[i], y = col_names[j])

    ggsave(filename = paste0(col_names[j], "_VS_", col_names[i],".pdf"),
           plot = current_plot,
           path = "Response curves plots 10_05_2022/",
           device = "pdf")
  ...
  # removing all NA values from diagnostic df
  
  diagnostic_df <- na.omit(diagnostic_df)
  
  # generating descriptive stats
  diagnostic_df %>% 
    filter(relationship == "Fraud relationship") %>% 
    ggplot(aes(x = decision_boundry_strength)) +
    geom_histogram()
