...
accounts_sim <- read.csv("sample/accounts.csv") 
transactions_sim <- read.csv("sample/transactions.csv") 
alert_transactions_sim <- read.csv("sample/alert_transactions.csv")
alert_acc_sim <- read.csv("sample/alert_accounts.csv")
cash_tx_sim <- read.csv("sample/cash_tx.csv")
ind_bulk_sim <- read.csv("sample/individuals-bulkload.csv")
org_bulk_sim <- read.csv("sample/organizations-bulkload.csv")
resolved_ent_sim <- read.csv("sample/resolvedentities.csv")
sar_acc_sim <- read.csv("sample/sar_accounts.csv")
...
transaction_feature_function <- function(transaction_df, accounts_df){ 
  ...
  # 3. Creating transaction feature df
...
#### TRAINING DATA - first data set generated ####

transactions_data_final_train <- transactions_data_final
accounts_data_final_train <- accounts_data_final


# 1. Extracting transaction features

transaction_feature_df_train <- transaction_feature_function(transaction_df = transactions_data_final_train, accounts_df = accounts_data_final_train)

transaction_feature_df_train$client_ID <- as.character(transaction_feature_df_train$client_ID)
...
undirected_weighted_graph_raw_train <- graph_from_data_frame(d = transactions_data_final_train,
                                                       directed = F,
                                                       vertices = accounts_data_final_train) 
  
# Simplifying the graph (removing multiple edges)
undirected_weighted_graph_simp_train <- simplify(undirected_weighted_graph_raw_train,
                                           remove.multiple = T,
                                           remove.loops = T,
...
weight_ind <- which(names(transactions_data_final_train) == 'weight')

directed_graph_train <- graph_from_data_frame(d = transactions_data_final_train[,-weight_ind],
                                       directed = T,
                                       vertices = accounts_data_final_train)


# 3. Extract the network features for each component in the graph

# NB: This should be a check since the simulator should produce only a single component

# 3.1 Un-directed weighted graph

# decomposing graph into its graph components
undirected_weighted_graph_components_train <- decompose.graph(graph = undirected_weighted_graph_simp_train,
                                                 min.vertices = 3) 

# Note: Only one component was identified

# Extracting graph components
current_undirected_weighted_component_train <- undirected_weighted_graph_components_train[[1]]

# Note: The number of vertices changed

# extracting features

undirected_weighted_graph_features_train <- undirected_weighted_network_feature_func(sub_graph = current_undirected_weighted_component_train,
                                                                                verbose = T)

# Note: Takes about 4.5 mins

# 3.2 Directed graph

# decomposing graph into its graph components
directed_graph_components_train <- decompose.graph(graph = directed_graph_train,
                                                 min.vertices = 3) 
# Note: The number of vertices changed

# extracting graph components
current_directed_component_train <- directed_graph_components_train[[1]]


# extracting features


directed_graph_features_train <- directed_network_feature_function(sub_graph = current_directed_component_train,
                                                             verbose = T)

# Note: Takes about 3 mins


# 4. Combing generated data tables into one feature df

fraud_df_train <- data.frame(client_ID = as.character(accounts_data_final_train$client_ID),
                          is_fraud = accounts_data_final_train$is_fraud)

final_feature_df_train <- left_join(undirected_weighted_graph_features_train, directed_graph_features_train, by = "client_ID") %>% 
  left_join(transaction_feature_df_train, by = "client_ID") %>% 
  left_join(fraud_df_train, by = "client_ID")


#### TESTING DATA- second data set generate ####
