...
node_df <- data.frame(Client_ID = LETTERS[1:20],
                      is_fraud = c(rep(F, 3), T, T, T, rep(F,2), T, rep(F,11))) # Added additional 7 nodes to build in some sparsity in network
...
Network feature generation function
...
*input:* a sub-graph with more than two nodes

*output:* a data frame containing all the network metrics extracted from the sub-graph. 

NB! The function will be able to take as input i) un-directed-weighted networks (most features) ii) directed network (limited features) 

```{r, network feature generation function}
...
# 1.1.8 Probabilistic relational neighbor classifier
  ...
  # Note: df will be joined to network feature df at the end of function.
